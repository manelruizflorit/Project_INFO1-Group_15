...
def assign_night_gates():
    ...
    if bcn_airport is None:
        messagebox.showerror("Error", "Airport structure not loaded. Please load LEBL airport first.")
        return
    night = NightAircraft(my_departures)
    if night == -1:
...
# Checks the status (free/occupied) of a gate by its ID
def id_search():
    global bcn_airport
    if bcn_airport is None:
        messagebox.showerror("Error", "Airport structure not loaded. Please load LEBL airport first.")
    ...
    gate_id_to_find = entry_gate_search.get().strip().upper()
    if not gate_id_to_find:
        messagebox.showwarning("Warning", "Please enter a gate ID to search.")
        return
    ...
    for terminal in bcn_airport.terminals:
        for area in terminal.boarding_areas:
            for gate in area.gates:
                if gate.name.upper() == gate_id_to_find:
                    found = True
                    if gate.occupied:
                        messagebox.showinfo("Gate Status",
                                            f"Gate {gate.name} is OCCUPIED.\nAircraft ID: {gate.aircraft_id}")
                    else:
                        messagebox.showinfo("Gate Status", f"Gate {gate.name} is FREE.")
                    break
            if found:
                break
        if found:
    ...
    if not found:
        messagebox.showerror("Not Found", f"Gate '{gate_id_to_find}' does not exist in {bcn_airport.code}.")


# Counts the number of departures before the hour entered in entry_hour
def count_departures_before():
    try:
        hour = int(entry_hour.get())
    except ValueError:
        messagebox.showerror("Error", "Please enter a valid hour (0-23).")
        return
    count = 0
    i = 0
    while i < len(my_departures):
        ...
        if departure_hour < hour:
            count += 1
        i += 1
    messagebox.showinfo("Result", f"Number of departures before {hour:02d}:00: {count}")
...
tk.Button(frame_flights, text="Long Distance Flights on Map", width=42, command=long_distance_flights).grid(row=4, column=0, columnspan=2, pady=5)
tk.Button(frame_flights, text="Save Arrivals", width=42, command=save_flights).grid(row=5, column=0, columnspan=2)

tk.Label(frame_flights, text="Departures before hour:").grid(row=6, column=0, sticky="e", padx=5, pady=5)
entry_hour = tk.Entry(frame_flights, width=5)
entry_hour.grid(row=6, column=1, sticky="w", padx=5)
tk.Button(frame_flights, text="Count", width=10, command=count_departures_before).grid(row=7, column=0, columnspan=2, pady=3)
...
tk.Frame(frame_gates, height=2, bd=1, relief="sunken").grid(row=1, column=0, columnspan=4, sticky="ew", pady=10)

tk.Label(frame_gates, text="Search Gate ID:").grid(row=2, column=0, sticky="e", padx=5)
entry_gate_search = tk.Entry(frame_gates, width=15)
entry_gate_search.grid(row=2, column=1, sticky="w", padx=5)
tk.Button(frame_gates, text="Check Status", width=15, bg="lightblue", command=id_search).grid(row=2, column=2, sticky="w", padx=5)
